...
namespace CppTestHarness {
	...
	void DllManager::Load(const CppUtils::Cpp_string& name) {
		this->Unload();
		try {
	...
	void DllManager::Unload() {
		try {
			...
			this->m_dllHandle = DLL_HANDLE_NULL;
			throw CppUtils::Cpp_exception(_FL_, _L_("Exception on closing dll handle"));
		}
	}
	...
	// need to use the other regular handle in order to invoke the export functions
		HMODULE handle = LoadLibraryEx (CppUtils::WideToNarrowString(dllName).c_str(), NULL, DONT_RESOLVE_DLL_REFERENCES);
		...
		for (unsigned int i = 0; i < exports->NumberOfNames; i++) {
			...
			if (CppUtils::Contains(name, "_this_is_a_CppTest_loader_")) {
