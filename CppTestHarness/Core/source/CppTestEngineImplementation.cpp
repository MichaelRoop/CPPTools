...
namespace CppTestHarness {
	...
	public:
		...
		bool operator () (IFixture* fixture) {
			CppUtils::Cpp_pointerException::ptrAssert(fixture, _FL_);
	...
	public:
		HasNamedFixtureFunctor(const CppUtils::Cpp_string& name) : m_name(name) {
		}
	...
	public:
		BuildTestCaseList(std::vector<CppUtils::CppSharedPtr<IFixutureTestCaseNames> >& names)
			: m_names(names) {
		}

		void operator () (IFixture* fixture) {
			CppUtils::Cpp_pointerException::ptrAssert(fixture, _FL_);
			this->m_names.push_back(fixture->GetTestNames());
		}
	private:
		std::vector<CppUtils::CppSharedPtr<IFixutureTestCaseNames> >& m_names;
	};
	...
	public:
		FireLoggedEventsFunctor(const ICase& testCase) : m_case(testCase) {
		}

		void operator () (const DataLoggedEvent logEvent) {
	...
	public:
		FireSummaryEventsFunctor(const CaseCounter& caseCounter) {
			this->m_summary = new RunSummary(caseCounter);
		}

		void operator () (const TestRunSummaryData summaryEvent) {
			//mr_utils::mr_pointerException::ptrAssert(static_cast<void*>(summaryEvent), _FL_ );
			(*summaryEvent)((*this->m_summary));
		}

		CppUtils::CppSharedPtr<IRunSummary> m_summary;
	};
	...
	class NonExistantTestData : public Case {
	public:
		/// @brief	Constructor.
		/// @param	testName	Unique name for the test.
		NonExistantTestData(const CppUtils::Cpp_string& fixtureName, const CppUtils::Cpp_string& name)
			: Case(fixtureName, name, L("Test not found")) {
			this->Status = ICase::TestCaseStatus::ST_NOT_EXISTS;
		}
	};


	/// @brief Dummy test case to report to the loggers that the names test case was not found
	class DisabledTestData : public Case {
	public:
		/// @brief	Constructor.
		/// @param	testName	Unique name for the test.
		DisabledTestData(const CppUtils::Cpp_string& fixtureName, const CppUtils::Cpp_string& name)
			: Case(fixtureName, name, L("Test Disabled")) {
			this->Status = ICase::TestCaseStatus::ST_DISABLED;
		}
	};


	EngineImplementation::EngineImplementation() {
		this->m_testDllManger = new DllManager();
	}


	void EngineImplementation::RegisterCase(IFixture* fixture) {
		CppUtils::Cpp_pointerException::ptrAssert(fixture, _FL_);
	...
	void EngineImplementation::LoadTests(const CppUtils::Cpp_string& dllName) {
	...
	void EngineImplementation::ProcessTestList(std::vector<CppUtils::CppSharedPtr<ITestFixtureInfoObject> >& list) {
		this->m_caseCounter.Reset();
		this->m_logEngine.WriteHeaders();

		// Iterate through each Fixture info
		std::vector< CppUtils::CppSharedPtr<ITestFixtureInfoObject> >::iterator itFixtureInfo = list.begin();
		for (; itFixtureInfo != list.end(); itFixtureInfo++) {

			// Only look up the actual fixture if the Fixture Info has test case infos
			const std::vector<TestInfoObject>& testInfos = (*itFixtureInfo)->GetTestInfoVector();
	...
	void EngineImplementation::LogResults(ICase& testCase) {
	...
	CppUtils::Cpp_string EngineImplementation::GetRunId() {
		if (m_runId.empty()) {
			time_t		rawtime;
			struct tm   t;
			char		buff[200];
			...
			time(&rawtime);
			// TODO - not checking error return
			localtime_s(&t, &rawtime);
			strftime(buff, 200, "%Y%m%d%H%M%S", (const tm*) &t);

			CppUtils::Cpp_stringstream os;
			os << buff;
			m_runId = os.str();
		}
		return m_runId;
	...
	std::vector<CppUtils::CppSharedPtr<IFixutureTestCaseNames> > EngineImplementation::GetTestNames() {
		std::vector<CppUtils::CppSharedPtr<IFixutureTestCaseNames> > testNames;
		std::for_each(this->m_fixtures.begin(), this->m_fixtures.end(), BuildTestCaseList(testNames));
		return testNames;
	}


	void EngineImplementation::RegisterLoggedEvent(DataLoggedEvent loggedCallbackEvent) {
	...
	void EngineImplementation::RegisterSummaryEvent(TestRunSummaryData summrayCallbackEvent) {
	...
	void EngineImplementation::LoadLoggersByFileDefinition(const CppUtils::Cpp_string& fileName, const CppUtils::Cpp_string& fileType) {
		this->m_logEngine.LoadLoggers(fileName, fileType);
	}


	void EngineImplementation::RunSelectFixtureTests(const CppUtils::Cpp_string& fixtureName, const std::vector<TestInfoObject>& testInfos) {

		// Retrieve the actual fixture pointer and check if registered
		std::vector<IFixture*>::iterator itFixture = this->GetFixtureIterator(fixtureName);
		if (itFixture == this->m_fixtures.end()) {
			...
			std::vector<TestInfoObject>::const_iterator itTestInfo = testInfos.begin();
			for (; itTestInfo != testInfos.end(); ++itTestInfo) {

				CppUtils::Cpp_string name = itTestInfo->GetName();
	...
	void EngineImplementation::RunAllFixtureTests(const CppUtils::Cpp_string& fixtureName) {
		...
		if (itFixture == this->m_fixtures.end()) {
			...
			std::vector<CppUtils::Cpp_string>::const_iterator itTestNames = (*itFixture)->GetTestNames()->TestCaseNames().begin();
			std::vector<CppUtils::Cpp_string>::const_iterator endTestNames = (*itFixture)->GetTestNames()->TestCaseNames().end();
			for (; itTestNames != endTestNames; ++itTestNames) {
				if ((*itFixture)->HasTest(*itTestNames)) {
					(*itFixture)->RunTest((*itTestNames), TestCaseArguments());
					this->LogResults((*itFixture)->CurrentTestCase());
				}
				else {
	...
	std::vector<IFixture*>::iterator EngineImplementation::GetFixtureIterator(const CppUtils::Cpp_string& fixtureName) {
		return std::find_if(
			this->m_fixtures.begin(),
			this->m_fixtures.end(),
