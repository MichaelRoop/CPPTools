...
namespace CppTestHarness {
	...
	OdbcOutput::OdbcOutput(CppSharedPtr<ILogInitialiser>& initialiser)
		:
	...
	bool OdbcOutput::Write(const Cpp_string& str) {
#if defined (_WIN32) || defined(_WIN64)
		Cpp_exception::assertCondition(this->m_odbcEnv != SQL_NULL_HENV, _FL_, _L_("No DB Connection"));
		...
		if (ret == SQL_ERROR) {
			...
			while (SQLError(this->m_odbcEnv, this->m_odbcHdbc, stmt, sqlstate, &sqlcode, buffer,
				...
				Cpp_cout << std::endl << L("**** ERROR *****") << std::endl;
				Cpp_cout << L("SQLSTATE:") << sqlstate << std::endl;
				Cpp_cout << L("Native Error Code:") << sqlcode << std::endl;
				Cpp_cout << buffer << std::endl;
			};
		}
