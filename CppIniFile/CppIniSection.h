#if !defined(CPP_INI_SECTION_H)
#define CPP_INI_SECTION_H
...
namespace CppIniFileNs {
	...
	public:
		...
		typedef std::vector<CppIniNode> NodeVector;

		/// @typedef	Definition of a node vector iterator.
		typedef std::vector<CppIniNode>::const_iterator	NodeIterator;
		...
		const Cpp_string& value(const Cpp_string& nodeName) const;
		...
		void SetName(const Cpp_string& name);
		...
		NodeIterator GetNode(const Cpp_string& name) const;
		...
		bool NodeExists(const Cpp_string& name) const;
	...
	private:
		...
		Cpp_string m_dummyVal;	///< Empty dummy value string for not found. 
	...
	/// <returns></returns>
	Cpp_ostream& operator << (Cpp_ostream& os, const CppIniSection& theSection);
